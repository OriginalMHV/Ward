...
Returns per-repo: project type, language, versions, security features, config files, alert counts by severity, and dependency graph / SBOM audit data. The `dependency_graph` block is the primary signal for whether GitHub currently has usable dependency data for a repository. If that signal is missing, Ward also includes optional dependency-submission workflow diagnostics to help explain why.
